...
words_per_line = 5
lines_list = [
    random_words[i * words_per_line:(i + 1) * words_per_line] for i in range(num_lines)
]
...
def refresh_text_box():
    ...
    text_box.insert(INSERT, "\n".join([" ".join(item) for item in lines_list[line_index:line_index+2]]))
...
root.bind("<Key>", lambda e: start_countdown() if e.char.isalpha() and not countdown_started else None)


root.bind("<Key>", lambda e: compare_letters(current_word) if e.char.isalpha() else None)

# TODO: Need to solve backspace with one space + word color when using backspace
root.bind("<BackSpace>", lambda e: backspace_action() if word_index > 0 and entry_text.get() == '' else None)

root.bind("<space>", lambda e: space_action() if not entry_text.get().isspace() else None)
